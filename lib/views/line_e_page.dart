...
class LineEPage extends StatefulWidget {
  final DateTime date;
  const LineEPage({Key? key, required this.date}) : super(key: key);

  @override
  _LineEPageState createState() => _LineEPageState();
}
...
class _LineEPageState extends State<LineEPage> {
  ...
  void _processTargetMap(Map<String, dynamic>? targetMap) {
    print('Processing target map for Line E');
  ...
  void _setupStreams() {
    ...
    print('Setting up streams for date: $dateStr, Line E');
    ...
    _planSubscription = _firestore.collection('counter_sistem')
      ...
      .listen((DocumentSnapshot snapshot) {
        if (snapshot.exists) {
          ...
          final plan = data?['target_E'] as int? ?? 0;
          ...
          // Process target_map_E for real-time calculation
          final targetMap = data?['target_map_E'] as Map<String, dynamic>?;
    ...
    _kumitateSubscription = kumitateDocRef.snapshots().listen((DocumentSnapshot snapshot) {
      if (snapshot.exists) {
        ...
        print('Contract collections for Line E: $contractCollections');
