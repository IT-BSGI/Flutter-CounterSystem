...
class _LineBPageState extends State<LineBPage> {
  ...
  void _processTargetMap(Map<String, dynamic>? targetMap) {
    print('Processing target map for Line B');
  ...
  void _setupStreams() {
    ...
    print('Setting up streams for date: $dateStr, Line B');
    ...
    _planSubscription = _firestore.collection('counter_sistem')
      ...
      .listen((DocumentSnapshot snapshot) {
        if (snapshot.exists) {
          ...
          final plan = data?['target_B'] as int? ?? 0;
          ...
          // Process target_map_B for real-time calculation
          final targetMap = data?['target_map_B'] as Map<String, dynamic>?;
    ...
    _kumitateSubscription = kumitateDocRef.snapshots().listen((DocumentSnapshot snapshot) {
      if (snapshot.exists) {
        ...
        print('Contract collections for Line B: $contractCollections');
